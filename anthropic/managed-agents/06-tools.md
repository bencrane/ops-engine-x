...
In addition to built-in tools, you can define custom tools. Custom tools are analogous to [user-defined client tools](https://docs.anthropic.com/docs/en/agents-and-tools/tool-use/how-tool-use-works#user-defined-tools-client-executed) in the Messages API.
...
Once you've defined the tool at the agent level, the agent will invoke the tools through the course of a session. See [Session event stream](https://docs.anthropic.com/docs/en/managed-agents/events-and-streaming#handling-custom-tool-calls) for the full flow.
