...
- For US-only inference (`inference_geo: "us"`) requests on Claude Opus 4.6 and newer models, input tokens are 1.1 tokens per token
...
- For US-only inference (`inference_geo: "us"`) requests on Claude Opus 4.6 and newer models, output tokens are 1.1 tokens per token
...
Check the model overview page for more details on available models.
