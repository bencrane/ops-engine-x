...
2. **Session creation** supplies auth for those servers by referencing a pre-registered [vault](https://docs.anthropic.com/docs/en/managed-agents/vaults).
...
> **Note:** All Managed Agents API requests require the `managed-agents-2026-04-01` beta header. The SDK sets the beta header automatically.
...
> **Tip:** The MCP toolset defaults to a permission policy of `always_ask`, which requires user approval before each tool call. See [permission policies](https://docs.anthropic.com/docs/en/managed-agents/permission-policies) to configure this behavior.

## Provide auth at session creation

When starting a session, pass `vault_ids` to provide credentials for your MCP servers. Vaults are collections of credentials that you register once and reference by ID. See [Authenticate with vaults](https://docs.anthropic.com/docs/en/managed-agents/vaults) for how to create vaults and manage credentials.
...
Claude Managed Agents connects to [remote MCP servers](https://docs.anthropic.com/docs/en/agents-and-tools/remote-mcp-servers) that expose an HTTP endpoint. The server must support the MCP protocol's streamable HTTP transport.
