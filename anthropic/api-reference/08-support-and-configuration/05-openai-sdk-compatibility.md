...
>
> If you encounter any issues with the OpenAI SDK compatibility feature, please share your feedback via the [compatibility feedback form](https://forms.gle/oQV4McQNiuuNbz9n8).

> **Tip:** For the best experience and access to Claude API full feature set (PDF processing, citations, extended thinking, and prompt caching), use the native Claude API.
...
**`functions[n]` fields** (deprecated by OpenAI, use `tools` instead)
...
**Developer role** (`messages[n].role == "developer"`)
...
**System role** (`messages[n].role == "system"`)
...
**User role** (`messages[n].role == "user"`)
...
**Assistant role** (`messages[n].role == "assistant"`)
...
**Tool role** (`messages[n].role == "tool"`)
...
**Function role** (`messages[n].role == "function"`)
