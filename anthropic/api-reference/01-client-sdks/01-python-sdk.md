...
Consider using `python-dotenv` to add `ANTHROPIC_API_KEY="my-anthropic-api-key"` to your `.env` file so that your API key isn't stored in source control.
...
Nested request parameters are `TypedDict`s. Responses are Pydantic models which also have helper methods for things like serializing back into JSON (v1, v2).
