...
### Phase 3 — Build managed-agents-x-api (WHEN READY)
...
12. **Extract Anthropic-calling code** from ops-engine-x → new repo.
...
10. **Token-naming convention.** A token's name reflects the service it grants access *to*, not the service holding it. Inbound-to-ops-engine-x = `OPEX_AUTH_TOKEN`. Outbound-from-ops-engine-x to service X = `X_AUTH_TOKEN` (e.g. `MAG_AUTH_TOKEN` when calling managed-agents-x-api).
